...
import {
  FileText, Plus, Trash2, Loader2, AlertTriangle, Users, Target, CheckCircle, XCircle
} from 'lucide-react';
import { db } from '@/lib/firebase';
import {
  collection, query, onSnapshot, addDoc, deleteDoc, doc, writeBatch, getDocs, collectionGroup
...
export default function WorkoutTemplatesPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      query(collection(db, 'workoutTemplates')),
      (snap) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      router.push(`/coach/workout-templates/${docRef.id}`);
    } catch {
  ...
  const handleDelete = async (templateId: string) => {
    ...
    try {
      ...
      const daysSnap = await getDocs(collection(db, 'workoutTemplates', templateId, 'days'));
      for (const dayDoc of daysSnap.docs) {
        const exercisesSnap = await getDocs(collection(db, 'workoutTemplates', templateId, 'days', dayDoc.id, 'exercises'));
        exercisesSnap.forEach((ex) => batch.delete(ex.ref));
        batch.delete(dayDoc.ref);
      }
      batch.delete(doc(db, 'workoutTemplates', templateId));
      await batch.commit();
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-display font-bold text-3xl md:text-4xl uppercase tracking-wider">
            Workout <span className="text-brand-blue">Templates</span>
          </h1>
          <p className="text-gray-400 text-sm mt-1">Manage coach-designed workout templates.</p>
        </div>
        <button
          id="new-template-btn"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-brand-dark border border-brand-gray rounded-3xl">
          <FileText size={48} className="text-brand-blue/30 mb-4" />
          <p className="text-gray-500 font-display uppercase tracking-widest text-lg mb-2">No workout templates available yet.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((t) => (
            <div
              ...
              onClick={() => router.push(`/coach/workout-templates/${t.id}`)}
            >
              <div className="flex items-start justify-between gap-3 mb-4">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500 font-mono mb-4">
                ...
                <span className="flex items-center gap-1.5">
                  <FileText size={12} className="text-brand-blue/70" /> {t.daysCount} day{t.daysCount !== 1 ? 's' : ''}
                </span>
              </div>
