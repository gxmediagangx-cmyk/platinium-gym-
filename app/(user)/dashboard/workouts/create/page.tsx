...
import {
  Dumbbell, FileText, Check, ArrowLeft, ArrowRight,
  Loader2, AlertTriangle, Users, Target, Star
} from 'lucide-react';
import { auth, db } from '@/lib/firebase';
import {
  collection, query, where, orderBy, getDocs, addDoc,
  doc, serverTimestamp, Timestamp, writeBatch, updateDoc
...
export default function CreateWorkoutPage() {
  ...
  useEffect(() => {
    if (step === 2 && workoutType === 'template' && user) {
  ...
  }, [step, workoutType, user]);

  const canGoNext = (): boolean => {
    if (step === 1) return workoutType !== null;
    if (step === 2 && workoutType === 'custom') return name.trim().length > 0;
    if (step === 2 && workoutType === 'template') return selectedTemplate !== null;
    return true;
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (workoutType === 'custom') {
        docRef = await addDoc(collection(db, 'users', user.uid, 'workouts'), {
          ...baseData,
          name: name.trim(),
          description: description.trim() || '',
          type: 'custom',
        });
      } else if (workoutType === 'template' && selectedTemplate) {
        docRef = await addDoc(collection(db, 'users', user.uid, 'workouts'), {
          ...baseData,
        ...
        // Copy template days + exercises into the new workout
        const daysSnap = await getDocs(
          query(
            collection(db, 'workoutTemplates', selectedTemplate.id, 'days'),
            orderBy('order', 'asc')
          )
        );
        ...
        if (!daysSnap.empty) {
          ...
          for (const dayDoc of daysSnap.docs) {
            ...
            const newDayRef = doc(collection(db, 'users', user.uid, 'workouts', docRef.id, 'days'));
            batch.set(newDayRef, {
              ...dayData,
              createdAt: serverTimestamp(),
            });
            ...
            const exercisesSnap = await getDocs(
              query(
                collection(db, 'workoutTemplates', selectedTemplate.id, 'days', dayDoc.id, 'exercises'),
                orderBy('order', 'asc')
              )
            );
            exercisesSnap.forEach((exerciseDoc) => {
              const exerciseData = exerciseDoc.data();
              const newExerciseRef = doc(collection(db, 'users', user.uid, 'workouts', docRef.id, 'days', newDayRef.id, 'exercises'));
          ...
          // Update daysCount
          await updateDoc(docRef, { daysCount: copiedDays });
        }
      } else {
        throw new Error('Invalid workout configuration');
      }

      router.push(`/dashboard/workouts/${docRef.id}`);
    } catch (err: any) {
      setSubmitError(err.message || 'Failed to create workout');
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto pb-24">
      ...
      {step === 1 && (
        <div className="animate-fade-in">
          <h1 className="font-display font-extrabold text-3xl md:text-5xl uppercase tracking-wider mb-2">
            Create <span className="text-brand-blue">Workout</span>
          </h1>
          <p className="text-gray-400 mb-10">Choose how you want to build your training system.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="w-14 h-14 rounded-2xl bg-brand-blue/10 flex items-center justify-center mb-5">
                <Dumbbell size={28} className="text-brand-blue" />
              </div>
              <h3 className="font-display font-bold text-2xl uppercase tracking-wide text-white mb-2">Custom Workout</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                Build your own workout system from scratch. Name it, describe it, and organize your training your way.
              </p>
              {workoutType === 'custom' && (
            ...
            >
              ...
              <p className="text-sm text-gray-400 leading-relaxed">
                Start with a professionally designed template from our elite coaches. Customize it to fit your goals.
              </p>
              {workoutType === 'template' && (
                <div className="mt-4 flex items-center gap-2 text-brand-blue text-xs font-mono uppercase tracking-widest">
                  <Check size={14} /> Selected
            ...
            <div className="flex flex-col gap-6">
              <div>
                <label className="block text-[10px] uppercase font-mono tracking-wider text-gray-400 mb-2">
                  Workout Name <span className="text-brand-blue">*</span>
                </label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe the workout system..."
                  maxLength={200}
                />
                <div className="text-right text-[10px] font-mono text-gray-500 mt-1">
      ...
      {step === 3 && (
        <div className="animate-fade-in">
          <h2 className="font-display font-bold text-2xl md:text-3xl uppercase tracking-wider mb-2">
            Confirm <span className="text-brand-blue">Workout</span>
          </h2>
          <p className="text-gray-400 mb-8">Review the details before creating your workout system.</p>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-brand-black/50 border border-brand-gray-light">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                  workoutType === 'custom' ? 'bg-brand-blue/10' : 'bg-purple-500/10'
                }`}>
                  {workoutType === 'custom' ? (
                ...
                <div>
                  ...
                  <div className="text-sm font-bold text-white uppercase tracking-wider">
                    {workoutType === 'custom' ? 'Custom Workout' : 'Coach Template'}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-brand-black/50 border border-brand-gray-light">
                ...
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider font-mono">
                    {workoutType === 'custom' ? 'Name' : 'Template'}
                  </div>
                  <div className="text-sm font-bold text-white break-words">
                    {workoutType === 'custom'
                      ? name.trim() || 'Untitled Workout'
                      : selectedTemplate?.name || 'Unknown Template'
                    }
                  </div>
                  {workoutType === 'template' && selectedTemplate?.coachName && (
              ...
              {workoutType === 'custom' && description.trim() && (
              ...
              {workoutType === 'template' && selectedTemplate && (
